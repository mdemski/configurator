...
export class AccessoriesComponent implements OnInit, OnDestroy {
  ...
  accessoriesList: { product: Accessory, id: number }[] = [];
  filteredAccessoriesList: { product: Accessory, id: number }[] = [];
  searchText: string;
  isFiltering = false;
  page = 1;
  pageSize = 10;
  ...
  ngOnInit(): void {
    ...
    this.accessories$.pipe(takeUntil(this.isDestroyed$)).subscribe(accessories => {
      accessories.forEach((product, index) => {
        this.accessoriesList.push({product, id: index});
      });
    });
  ...
  filterInput(filtersObject: any) {
    ...
    let filteredByType: { product: Accessory, id: number }[];
    let numberOfKindNull = 0;
    let filteredByKind: { product: Accessory, id: number }[];
    let numberOfMaterialTypeNull = 0;
    let filteredByMaterialType: { product: Accessory, id: number }[];
    let numberOfMaterialMatchingNull = 0;
    let filteredByFrameMatching: { product: Accessory, id: number }[];
    let numberOfMaterialColorNull = 0;
    let filteredByMaterialColor: { product: Accessory, id: number }[];
    ...
    if (numberOfTypeNull === this.filters.accessoryType.length) {
      ...
      filteredByType = this.accessoriesList
        .filter(accessory => this.filters.accessoryType.includes(accessory.product.typ));
    }
    if (numberOfKindNull === this.filters.accessoryKind.length) {
      filteredByKind = filteredByType;
    } else {
      filteredByKind = filteredByType
        .filter(accessory => this.filters.accessoryKind.includes(accessory.product.rodzaj));
    }
    if (numberOfMaterialTypeNull === this.filters.accessoryMaterialType.length) {
      filteredByMaterialType = filteredByKind;
    } else {
      filteredByMaterialType = filteredByKind
        .filter(accessory => this.filters.accessoryMaterialType.includes(accessory.product.typTkaniny))
        .filter(accessory => this.filters.accessoryMaterialType.includes(accessory.product.oblachowanieMaterial))
        .filter(accessory => accessory.product.typTkaniny !== null || accessory.product.oblachowanieMaterial !== null);
    }
    if (numberOfMaterialMatchingNull === this.filters.accessoryFrameMatching.length) {
      filteredByFrameMatching = filteredByMaterialType;
    } else {
      filteredByFrameMatching = filteredByMaterialType
        .filter(accessory => this.filters.accessoryFrameMatching.includes(accessory.product.frameMarching));
    }
    if (numberOfMaterialColorNull === this.filters.accessoryMaterialColor.length) {
      filteredByMaterialColor = filteredByFrameMatching;
    } else {
      filteredByMaterialColor = filteredByFrameMatching
        .filter(accessory => this.filters.accessoryMaterialColor.includes(accessory.product.kolorTkaniny))
        .filter(accessory => this.filters.accessoryMaterialColor.includes(accessory.product.oblachowanieKolor))
        .filter(accessory => accessory.product.kolorTkaniny !== null || accessory.product.oblachowanieKolor !== null);
    }
    this.filteredAccessoriesList = filteredByMaterialColor.filter(accessory => {
      ...
      if (this.filters.accessoryName) {
        nameFound = accessory.product.productName.toString().trim().toLowerCase().indexOf(this.filters.accessoryName.toLowerCase()) !== -1;
      }
      if (this.filters.accessoryWidthFrom !== undefined && this.filters.accessoryWidthTo !== undefined) {
        ...
        if (this.filters.accessoryWidthFrom <= this.filters.accessoryWidthTo) {
          widthFound = accessory.product.szerokosc >= this.filters.accessoryWidthFrom && accessory.product.szerokosc <= this.filters.accessoryWidthTo;
        }
        if (this.filters.accessoryWidthTo < 20 && this.filters.accessoryWidthFrom > 10) {
          widthFound = accessory.product.szerokosc >= this.filters.accessoryWidthFrom;
        }
        if (this.filters.accessoryWidthFrom < 10 && this.filters.accessoryWidthTo > 20) {
          widthFound = accessory.product.szerokosc <= this.filters.accessoryWidthTo;
        }
        if (this.filters.accessoryWidthFrom < 10 && this.filters.accessoryWidthTo < 20) {
          widthFound = true;
        }
      ...
      if (this.filters.accessoryHeightFrom !== undefined && this.filters.accessoryHeightTo !== undefined) {
        ...
        if (this.filters.accessoryHeightFrom <= this.filters.accessoryHeightTo) {
          heightFound = accessory.product.wysokosc >= this.filters.accessoryHeightFrom && accessory.product.wysokosc <= this.filters.accessoryHeightTo;
        }
        if (this.filters.accessoryHeightTo < 20 && this.filters.accessoryHeightFrom > 10) {
          heightFound = accessory.product.wysokosc >= this.filters.accessoryHeightFrom;
        }
        if (this.filters.accessoryHeightFrom < 10 && this.filters.accessoryHeightTo > 20) {
          heightFound = accessory.product.wysokosc <= this.filters.accessoryHeightTo;
        }
        if (this.filters.accessoryHeightFrom < 10 && this.filters.accessoryHeightTo < 20) {
          heightFound = true;
        }
  ...
  sortArray() {
    switch (this.sortBy) {
      case 'popularity':
        this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.iloscSprzedanychRok'], ['asc']);
        break;
      case 'priceAsc':
        this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.CenaDetaliczna'], ['asc']);
        break;
      case 'priceDesc':
        this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.CenaDetaliczna'], ['desc']);
        break;
      case 'nameAsc':
        this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.productName'], ['asc']);
        break;
      case 'nameDesc':
        this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.productName'], ['desc']);
        break;
      default:
        this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.iloscSprzedanychRok'], ['asc']);
        break;
  ...
  sortByName() {
    ...
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.productName'], product);
  }

  sortByType() {
    this.typeToggler = !this.typeToggler;
    const product = this.typeToggler ? 'asc' : 'desc';
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.typ'], product);
  }

  sortByColor() {
    this.colorToggler = !this.colorToggler;
    const product = this.colorToggler ? 'asc' : 'desc';
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.kolorTkaniny'], product);
  }

  sortByWidth() {
    this.widthToggler = !this.widthToggler;
    const product = this.widthToggler ? 'asc' : 'desc';
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.szerokosc'], product);
  }

  sortByHeight() {
    this.heightToggler = !this.heightToggler;
    const product = this.heightToggler ? 'asc' : 'desc';
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.wysokosc'], product);
  }

  sortByMaterial() {
    this.materialToggler = !this.materialToggler;
    const product = this.materialToggler ? 'asc' : 'desc';
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.typTkaniny'], product);
  }
  ...
  sortByPrice() {
    ...
    this.filteredAccessoriesList = _.orderBy(this.filteredAccessoriesList, ['product.CenaDetaliczna'], product);
  }
  ...
  private filterTable(filterObject: { frameMatching: string; color: string; material: string; price: string; name: string; width: string; type: string; height: string }) {
    this.isFiltering = true;
    this.filteredAccessoriesList = this.accessoriesList;
    this.filteredAccessoriesList = this.filteredAccessoriesList.filter(data => {
      const accessory = data.product;
      let nameFound = true;
      let typeFound = true;
      let colorFound = true;
      let widthFound = true;
